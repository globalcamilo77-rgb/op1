...
export default function HomePage() {
  ...
  const whatsappContacts = useWhatsAppStore((state) => state.contacts)
  const whatsappDefaultMessage = useWhatsAppStore((state) => state.defaultMessage)
  ...
  // Pega o primeiro WhatsApp ativo (sem rotacao na home)
  const activeContact = mounted
    ? whatsappContacts.find(
        (c) => c.active && c.number.replace(/\D/g, '').length >= 10,
      )
    : null
  const contactNumber = activeContact?.number.replace(/\D/g, '') || ''

  const handleWhatsApp = () => {
    if (!contactNumber) return
    const message = `${whatsappDefaultMessage || 'Ola, gostaria de um orcamento.'}${
      name.trim() ? `\n\nMeu nome: ${name.trim()}` : ''
    }${postal.trim() ? `\nCEP: ${postal.trim()}` : ''}`
    trackEvent('lead', {
      meta: {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <div className="max-w-6xl mx-auto px-5 py-12 md:py-16 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-10 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur px-3 py-1.5 rounded-full text-xs font-semibold mb-4">
              <BadgeCheck size={14} /> Oferta especial por tempo limitado
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight">
              Material de construcao com{' '}
              <span className="text-[var(--orange-primary)]">entrega rapida</span> e melhor preco
            </h1>
            <p className="mt-4 text-lg text-white/90 leading-relaxed">
              Cimento, argamassa, rejunte e tudo para sua obra com descontos exclusivos. Frete
              reduzido e atendimento direto no WhatsApp.
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              >
                ...
                {contactNumber ? 'Falar no WhatsApp' : 'WhatsApp indisponivel'}
              </button>
            </div>
      ...
      <section className="bg-secondary py-12">
        <div className="max-w-6xl mx-auto px-5">
          <h2 className="text-2xl font-extrabold text-center mb-8">
            Por que escolher {headlineBrand}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-card p-6 rounded-xl text-center">
              ...
              <h3 className="font-bold">Entrega rapida</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Logistica dedicada para a sua obra com prazo medio de 48h.
              </p>
            </div>
            <div className="bg-card p-6 rounded-xl text-center">
              <div className="mx-auto w-12 h-12 rounded-full bg-[var(--orange-primary)]/15 text-[var(--orange-primary)] flex items-center justify-center mb-3">
      ...
      <section className="bg-[var(--orange-primary)] text-white py-12 px-5 text-center">
        <h2 className="text-3xl font-extrabold">Pronto pra comecar sua obra?</h2>
