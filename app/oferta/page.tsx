...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    text: 'Conseguir o melhor preco entre 4 lojas. O frete inclusive saiu mais barato do que retirar pessoalmente.',
  },
  {
    name: 'Joao P.',
...
export default function OfertaPage() {
  ...
  const defaultMessage = useWhatsAppStore((state) => state.defaultMessage)
  ...
  const headlineBrand = mounted ? `${brandName}${brandHighlight}` : `${DEFAULT_APPEARANCE.brandName}${DEFAULT_APPEARANCE.brandHighlight}`
  ...
  const handleWhatsApp = () => {
    ...
    const message = `${defaultMessage}\n\nVi a oferta no site.${
      name.trim() ? ` Meu nome e ${name.trim()}.` : ''
    }${postal.trim() ? ` CEP: ${postal.trim()}.` : ''}`
    trackEvent('lead', {
      meta: {
        type: 'whatsapp_click',
        source: 'oferta_lp',
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="bg-white text-foreground py-3 px-5 flex items-center justify-between gap-4 flex-wrap border-b border-border shadow-sm">
        ...
        <div className="flex items-center gap-2 text-sm font-semibold text-[var(--orange-primary)]">
          <Truck size={16} /> Entrega rapida em ate 48h
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto px-5 py-12 md:py-16 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-10 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur px-3 py-1.5 rounded-full text-xs font-semibold mb-4">
              <BadgeCheck size={14} /> Oferta limitada para esta semana
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight">
              Sua obra mais barata e mais rapida com {headlineBrand}
            </h1>
            <p className="mt-4 text-lg text-white/90 leading-relaxed">
              Ate <strong>22% off</strong> no cimento, argamassa e rejunte. Frete reduzido,
              parcelamento em ate 12x e atendimento direto pelo WhatsApp.
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 bg-[var(--success)] hover:bg-green-600 text-white px-5 py-3 rounded-full font-bold text-sm transition-colors"
              >
            ...
            {submitted ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Em breve um especialista da {headlineBrand} vai te chamar com a melhor proposta.
                </p>
                <button
                  onClick={handleWhatsApp}
              ...
              <form onSubmit={handleLeadSubmit} className="space-y-3">
                <div>
                  ...
                  <h3 className="text-2xl font-extrabold mt-1">
                    Te respondemos em <span className="text-[var(--orange-primary)]">10 minutos</span>
                  </h3>
                  <p className="text-xs text-muted-foreground mt-1">
                    Sem compromisso. Frete e prazo personalizados pra sua obra.
                  </p>
                </div>
      ...
      <section className="bg-secondary py-10">
        <div className="max-w-6xl mx-auto px-5 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Economia media na compra</p>
          </div>
          <div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Selecionamos os produtos com maior giro e melhores precos para a sua obra.
            </p>
          </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            {featured.length === 0 && (
              <div className="md:col-span-3 text-center text-sm text-muted-foreground py-10 bg-secondary rounded-lg">
                Cadastre produtos ativos no admin para que apareca aqui automaticamente.
              </div>
            )}
      ...
      <section className="bg-secondary py-12">
        <div className="max-w-6xl mx-auto px-5">
          <h2 className="text-2xl font-extrabold text-center mb-8">Porque escolher a {headlineBrand}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-card p-6 rounded-xl text-center">
              ...
              <h3 className="font-bold">Entrega 48h</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Estoque pronto e equipe logistica propria.
              </p>
            </div>
            <div className="bg-card p-6 rounded-xl text-center">
              <div className="mx-auto w-12 h-12 rounded-full bg-[var(--orange-primary)]/15 text-[var(--orange-primary)] flex items-center justify-center mb-3">
            ...
            <div className="bg-card p-6 rounded-xl text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Cartao, Pix ou boleto. 12x sem juros e Pix com 5% off.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[var(--orange-primary)] text-white py-12 px-5 text-center">
        <h2 className="text-3xl font-extrabold">Pronto para comecar?</h2>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <button
            onClick={handleWhatsApp}
            className="inline-flex items-center gap-2 bg-[var(--success)] hover:bg-green-600 text-white px-6 py-3 rounded-full font-bold text-sm"
          >
            <MessageCircle size={16} />
      ...
      <footer className="bg-[var(--graphite)] text-white/70 py-6 text-center text-xs">
        &copy; 2026 - {headlineBrand}. Todos os direitos reservados.
      </footer>
    </div>
  )
}
